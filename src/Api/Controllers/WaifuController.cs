...
namespace Sanakan.Api.Controllers
{
    ...
    public class WaifuController : ControllerBase
    {
        ...
        public async Task<IEnumerable<CardFinalView>> GetUsersCardsByShindenIdWithOffsetAndFilterAsync(ulong id, uint offset, uint count, [FromBody]CardsQueryFilter filter)
        {
            using (var db = new Database.UserContext(_config))
            {
                var user = await db.Users.AsQueryable().Where(x => x.Shinden == id).Include(x => x.GameDeck).AsNoTracking().FirstOrDefaultAsync();
                ...
                var query = db.Cards.AsQueryable().Where(x => x.GameDeckId == user.GameDeck.Id).Include(x=> x.ArenaStats).Include(x => x.TagList).AsNoTracking();
        ...
        public async Task<IEnumerable<CardFinalView>> GetUsersCardsByShindenIdWithOffsetAsync(ulong id, uint offset, uint count)
        {
            using (var db = new Database.UserContext(_config))
            {
                ...
                var cards = await db.Cards.AsQueryable().Where(x => x.GameDeckId == user.GameDeck.Id).Include(x=> x.ArenaStats).Include(x => x.TagList).Skip((int)offset).Take((int)count).AsNoTracking().ToListAsync();
                return cards.ToView();
            }
        }
        ...
        public async Task RepairCardsAsync(ulong oldId, ulong newId)
        {
            ...
            var exe = new Executable($"api-repair oc{oldId} c{newId}", new Task(() =>
            {
                using (var db = new Database.UserContext(_config))
                {
                    ...
                    var cards = db.Cards.AsQueryable().Where(x => x.Character == oldId);
        ...
        public async Task UpdateCardInfoAsync(ulong id, [FromBody]Models.CharacterCardInfoUpdate newData)
        {
            var exe = new Executable($"update cards-{id} img", new Task(() =>
            {
                ...
                {
                    var userRelease = new List<string>() { "users" };
                    var cards = db.Cards.AsQueryable().Where(x => x.Character == id);
